...
> On a microcontroller, where every byte counts, you can pin the size of an enum by naming its underlying type: `enum class ErrorCode : std::uint8_t { ... };` (the fixed-width type comes from `<cstdint>` in standard C++; Arduino code conventionally includes `<stdint.h>` and writes it without the `std::`. See [Arduino vs. Desktop C++](../arduino_vs_desktop.md)). You will not need this on the desktop.
